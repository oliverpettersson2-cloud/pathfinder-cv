...
pr:['Vilken utbildning passar mig baserat på [din bakgrund]?','Hitta YH-utbildningar inom vård i Helsingborg.','Hur ansöker jag till Komvux och vilka kurser behöver jag?']}
    ...
    if (currentTrainCat === 'intervju') {
      ...
      detail.innerHTML = `
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;">
          <button class="ov-back" onclick="trainBackToHub()" style="margin:0;">← Tillbaka till Träna</button>
  ...
  // ════════════════════════════════════════════════════════════════
  // INTERVJUTRÄNING — laddar befintlig intervju.js dynamiskt
  // (samma modul som mobile använder via window.ivInit)
  // ════════════════════════════════════════════════════════════════
  let _ivLoadPromise = null;
  function loadInterviewModule() {
    // Om ivInit redan finns — använd den direkt
    if (typeof window.ivInit === 'function') {
      try { window.ivInit('iv-container-train'); }
      catch(_) { try { window.ivInit(); } catch(__){} }
      return;
    }
    ...
    // Ladda intervju.js dynamiskt — pröva flera sökvägar
  ...
  function showInterviewLoadError() {
    const cont = document.getElementById('iv-container-train');
    if (!cont) return;
    cont.innerHTML = `
      <div style="text-align:center;padding:40px 20px;background:rgba(248,113,113,0.05);border:1px solid rgba(248,113,113,0.2);border-radius:12px;">
        ...
        <div style="font-size:13px;color:rgba(255,255,255,0.55);line-height:1.5;max-width:400px;margin:0 auto;">
          Kunde inte hitta <code style="background:rgba(255,255,255,0.06);padding:1px 5px;border-radius:3px;">intervju.js</code> på servern.
          Be utvecklaren att lägga till en <code style="background:rgba(255,255,255,0.06);padding:1px 5px;border-radius:3px;">&lt;script src="/intervju.js"&gt;&lt;/script&gt;</code> i HTML:en.
        </div>
